...
/*    fun getDouble(key: String?): Double {
    ...
    fun getListObject(key: String?): ArrayList<Book> {
        val gson = Gson()
        val objStrings = getListString(key)
        val playerList: ArrayList<Book> = ArrayList<Book>()
        for (jObjString in objStrings) {
            val player: Book = gson.fromJson(jObjString, Book::class.java)
    ...
    fun putListObject(key: String?, playerList: ArrayList<Book>) {
        checkForNullKey(key)
        val gson = Gson()
